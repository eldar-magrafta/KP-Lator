...
// Inverse of generateMeterEventHexString(): reads a type-7 event/alarm message.
// Per the NPR spec (Table 2) every message shares one 71-char KPackM layout;
// on Type 7 the Data field (positions 20-49) carries the event code in its
// last 2 chars. Layout:
//   STX D C <receiver(1)> <slot(1)> 7 <account(5)> <CurrentRead(8)> <Data(30)>
//   <channel(1)> <signal(2)> <repeater(2)> ETX <checksum(2)> <date(12)>
// The event code (e.g. AC = Air in Pipe) sits at the Data tail, mid(48,2).
...
//   STX D C <receiver> <slot> 7 <account(5 hex)> <36 zeros> <eventCode(2)> <channel> <signal(2)> <repeater(2)> ETX <checksum> <date(14)>
...
function generateMeterEventHexString() {
    try {
        ...
        // Current Read (12-19) + Data (20-49): per spec the Data field carries the
        // event code on Type 7. Zero-fill the first 36 chars, then the event code
        // occupies the last 2 chars of the Data field at positions 48-49.
        hexString += "0".repeat(36); // Current Read + leading Data (zero-filled for a pure event)
        hexString += code; // event code = last 2 chars of Data field, positions 48-49 (e.g. AC / BC)
        hexString += "0"; // channel
